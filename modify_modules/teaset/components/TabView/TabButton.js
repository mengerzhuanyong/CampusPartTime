...
export default class TabButton extends React.PureComponent {
  ...
  buildProps() {
    ...
    if (!React.isValidElement(icon) && (icon || icon === 0)) {
      ...
      icon = <Image resizeMode={'contain'} style={iconStyle} source={icon} />
    }
    ...
    if (!React.isValidElement(activeIcon) && (activeIcon || activeIcon === 0)) {
      ...
      activeIcon = <Image resizeMode={'contain'} style={iconStyle} source={activeIcon} />
    }
  ...
  render() {
    ...
    return (
      <TouchableOpacity activeOpacity={1.0} {...others}>
        {!useIcon ? null :
