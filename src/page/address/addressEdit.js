...
@inject('loginStore', 'shopStore', 'addressStore')
@observer
export default class AddressEdit extends Component {
    ...
    onSubmitAddress = async () => {
        ...
        let {item: {id}, username, mobile, area, address} = this.state;
        let url = ServicesApi.address_edit;
        let data = {
            id,
            username,
            mobile,
    ...
    render() {
        ...
        let {params} = this.props.navigation.state;
        let pageTitle = params && params.pageTitle ? params.pageTitle : '编辑地址';
        return (
